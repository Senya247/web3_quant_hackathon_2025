...
fn main() -> Result<()> {
    let strategy: FourierStrat = FourierStrat::create(1000.0, 0.01);
    let mut bt: BackTester = BackTester::create(strategy);

    let final_capital = bt.begin("../historical/BTCUSDT-1s-candles-2025-10.csv");

    println!("Final: {}", final_capital.unwrap());
    Ok(())
}
